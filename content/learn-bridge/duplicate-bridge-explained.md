...
og_description: ""
og_image: ""
canonical: "https://bridgeplaybook.com/learn-bridge/duplicate-bridge-explained/"
entity_tags:
  - duplicate bridge
  - matchpoints
  - bridge tournament
...
## The Core Concept: Removing Luck
...
This is what makes duplicate a game of pure skill.
...
- **Vulnerability** (pre-assigned: None, N/S, E/W, or Both)
- **Board number** (for tracking)

### Board Vulnerability Schedule

Board vulnerability is fixed by the Laws of Bridge:

| Boards | Vulnerability |
|--------|--------------|
| 1, 8, 11, 14, 17, 20, 23 | None |
| 2, 5, 12, 15, 18, 21 | N/S only |
| 3, 6, 9, 16, 19, 22 | E/W only |
| 4, 7, 10, 13 | Both |

## The Movement: How Boards Circulate

A standard club pairs session uses a **Mitchell movement**:

- **North/South pairs stay at their table** all session
- **East/West pairs move up one table** each round (table 1 → table 2 → table 3...)
- **Boards move down one table** each round (opposite to E/W)

Each round consists of 2–3 boards. After each round, E/W move, boards move, and play continues. By the end of the session, most pairs have played against most other pairs.

### Session Length

A typical club session:
- **24–27 boards** (8–9 rounds of 3 boards)
- **Duration**: approximately 2.5–3 hours
- **Scoring**: matchpoints or (less commonly) IMPs

## Matchpoint Scoring

After the session, scores for each board are compared across all pairs who played it in the same direction.

### How Matchpoints Work

For each board, you receive:
- **2 matchpoints** for each pair you outscored
- **1 matchpoint** for each pair you tied
- **0 matchpoints** for each pair who outscored you

**Example**: 7 North/South pairs play Board 4. You score +620 (game in 4♠). The other 6 pairs scored: +620, +620, +170, +170, +140, -50.

Your result vs others:
- Beat 3 pairs (+170, +140, -50): 6 matchpoints
- Tied 2 pairs (+620, +620): 2 matchpoints
- **Total: 8 matchpoints** out of a possible 12

### Top and Average

- **Top**: maximum matchpoints on one board (if 8 pairs, top = 14)
- **Average**: half of top (7 in this case)
- **Bottom**: zero matchpoints

A percentage of 60%+ is generally a good score; 55% wins most club sessions.

## Strategy Differences from Rubber Bridge
...
**Game bidding**: A 50% game is worth bidding in matchpoints (it gains or loses roughly the same). Thin slam bidding is riskier.

## Playing Your First Club Game
...
*See also: [bridge scoring](/learn-bridge/bridge-scoring/), [bridge rules](/learn-bridge/bridge-rules/), [contract bridge explained](/learn-bridge/contract-bridge-explained/).*
